...
class FlightConnection:
    ...
    def read_benchmarking_data(self) -> Generator:
        """Try to read data from flight service using the 'read_raw' parameter. This will allow to fetch binary data.
        Binary read should be used for small data, like json files, zip files etc. not for the big datasets as
        each data batch is joined to the previous one in-memory.
        """
        ...
        for endpoints in self.get_endpoints():
            for endpoint in endpoints:
                ...
                try:
                    ...
                    while True:
                        mini_batch, metadata = reader.read_chunk()
                        bytes_batch = mini_batch.columns[0].tolist()
                        binary_data_array.extend(bytes_batch)
                        array_size += len(bytes_batch[0])
                        if (
                            self.total_size_limit is not None
                            and array_size > self.total_size_limit
                        ):
                            raise MaxSizeOfFileExceeded(
                                array_size, self.total_size_limit
                            )
                except StopIteration:
                    pass

        bytes_array = b"".join(binary_data_array)

        file_content = bytes_array.decode("utf-8")
        json_content = json.loads(file_content)

        if isinstance(json_content, dict) and json_content.get("data") is not None:
            json_content = json_content.get("data")

        benchmarking_data = pd.json_normalize(json_content)

        for el_name in ["question", "correct_answer", "correct_answer_document_ids"]:
            if el_name not in benchmarking_data.columns.tolist():
                raise MissingMandatoryKey(el_name)

        if self.sampling_type == SamplingTypes.FIRST_VALUES:
            if self.total_nrows_limit is not None:
                benchmarking_data = benchmarking_data[: self.total_nrows_limit]
        elif self.sampling_type == SamplingTypes.RANDOM:
            benchmarking_data = benchmarking_data.sample(n=len(benchmarking_data))
            benchmarking_data = benchmarking_data.reset_index(drop=True)
        elif (
            self.sampling_type == SamplingTypes.LAST_VALUES
            and self.total_nrows_limit is not None
        ):
            benchmarking_data = benchmarking_data[-self.total_nrows_limit :]
            benchmarking_data = benchmarking_data.reset_index(drop=True)
        else:
            raise ValueError(
                f"Unsupported sampling type: {self.sampling_type} for benchmarking data."
            )

        yield benchmarking_data
