...
class DataConnection(BaseDataConnection):
    ...
    def _get_file_paths_from_bucket(self) -> list[str]:
        """Returns file paths (keys) of objects stored at a bucket location.

        Returns all file paths under ``self.location.get_location()`` prefix.
        First checks if prefix exists as a bucket "directory" - if not, treats prefix as file name.
        ...
        def get_keys_with_prefix(bucket_objects, prefix):
            return [
                ...
                if obj["Key"].startswith(prefix) and not obj["Key"].endswith("/")
            ]

        self._check_if_connection_asset_is_s3()
        cos_resource_client = self._init_cos_client()

        prefix = self.location.get_location().lstrip("/")
    ...
    def _get_connections_from_bucket(self) -> list["DataConnection"]:
        """Returns connections for every object in a bucket location.
        ...
        :rtype: list[DataConnection]
        """
        if not isinstance(self.location, S3Location):
    ...
    def from_dict(cls, connection_data: dict) -> "DataConnection":
        ...
        :param connection_data: a dictionary data structure with information about data connection reference
    ...
    def download(self, filename: str) -> None:
        """Download dataset stored in remote data storage and save to file.

        :param filename: path to file where data will be download
        :type filename: str
