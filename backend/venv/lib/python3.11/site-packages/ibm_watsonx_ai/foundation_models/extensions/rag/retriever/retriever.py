...
class Retriever(BaseRetriever):
    """Retriever class that handles retrieval operation for RAG implementation.
    Returns the `number_of_chunks` document segments using provided `method` based on relevant query in a ``retrieve`` method.

    :param vector_store: vector store to use for retrieval
    ...
    To create a repeatable retrieval using simple proximity search to return three nearest documents,
    create a `VectorStore` first. Then define a `Retriever`.
