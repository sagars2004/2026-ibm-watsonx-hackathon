...
class LangChainVectorStoreAdapter(BaseVectorStore):
    ...
    def _get_window_documents_chroma(
        ...
        and searches the vector_store according to the metadata.

        :param doc_id: document id
        :type doc_id: str

        :param seq_nums_window: a list of sequence numbers
        :type seq_nums_window: list[int]

        :return: a list of documents from that document with these sequence_numbers
        :rtype: list[Document]
        """
        expr = {
            "$and": [
    ...
    def _window_extend_and_merge(
        ...
        """
        Extend document (a chunk) to its adjacent chunks (if exist) from the same origin document.
        Then merge the adjacent chunks into one chunk while keeping their order,
        and merge intersecting text between them (if exists).
        This requires chunks to have "document_id" and "sequence_number" in their metadata.

        :param document: the document (chunk) to extend to its window, and merge
        ...
        match vs_type:
            ...
            case _:
                raise TypeError(
                    f"Currently we only support Milvus and Chroma. Received {type(self._langchain_vector_store)}."
                )
