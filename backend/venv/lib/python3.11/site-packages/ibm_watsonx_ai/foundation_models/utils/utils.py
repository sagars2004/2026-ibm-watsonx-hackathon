...
def _check_model_state(
    client: APIClient, model_id: str, tech_preview: bool = False
) -> None:
...
class _GenerateEventLoop:
    """
    Generate loop based on implementation of shared_event_loop
    from https://github.com/IBM/ibm-generative-ai
    """
    ...
    def __enter__(self) -> asyncio.AbstractEventLoop:
        ...
        def worker() -> None:
            ...
            for task in [
                t for t in asyncio.all_tasks(loop) if not (t.done() or t.cancelled())
            ]:
    ...
    def __exit__(
        ...
        if self.loop is not None:
            self.loop.call_soon_threadsafe(lambda: self.loop.stop())  # type: ignore[union-attr]
        self.thread.join()
...
generate_event_loop = _GenerateEventLoop()


def handle_shutdown_global(signal: int, frame: FrameType) -> None:
    generate_event_loop._handle_shutdown()
...
def _get_requests_session(thread_local: threading.local) -> requests.Session:
    """Initialize requests.Session in local scope for particular thread
    and return it."""
...
def _set_session_default_params(session: requests.Session) -> requests.Session:
    import requests as _requests

    # limit number of pool connections and pool size to 10
    adapter_http = _requests.adapters.HTTPAdapter(
        pool_block=True, pool_connections=10, pool_maxsize=10
    )
    session.mount("http://", adapter_http)
    adapter_https = _requests.adapters.HTTPAdapter(
        pool_block=True, pool_connections=10, pool_maxsize=10
    )
...
def _copy_function(func: Callable) -> Callable:
    ...
    new_func = functools.update_wrapper(new_func, func)  # type: ignore[assignment]
