...
class DocumentsIterableDataset(IterableDataset):
    ...
    :param api_client: Initialized APIClient object with set project or space ID. If DataConnection objects in list
     connections does not have set API client then the api_client object will be used for reading data.
            ...
            case "n_parallel":  # downloading documents entirely in parallel
                ...
                with mp.Pool(thread_no) as pool:
                    ...
                    for i in range(len(qs_output)):
                        result = qs_output[i].get()

                        if isinstance(result, Exception):
                            e = result
                            logger.error(e)
                            if self.error_callback:
                                self.error_callback(sampled_docs[i].document_id, e)
                        else:
                            doc = result
