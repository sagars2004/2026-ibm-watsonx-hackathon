...
class TabularIterableDataset(IterableDataset):
    ...
    :param stop_after_first_batch: if set to `True` the loading will stop after downloading first batch
    ...
    **Example: default sampling - read first 1GB of data**
    ...
    **Example: stratified/random sampling**
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.read_as_benchmark_data = (
            not self.binary_data
            and self._check_file_extension(
                connection=connection,
                expected_extensions=(".json", ".jsonl"),
    ...
    def _check_file_extension(
        ...
        """
        Checks whether DataConnection's Location references to one of expected

        :param connection: DataConnection object references to the remote file.
        :type connection: DataConnection

        :param expected_extensions: Sequence of file expected file extensions (each starts with dot), for example: `(".json", ".jsonl")`
        :type expected_extensions: Sequence[str]

        :param api_client: if connection is dict, api client is required to get details of the data connection
        :type expected_extensions: APIClient, optional

        :return: True if file extension could be recognized and it's  one of type in expected_extensions otherwise False
        """

        if isinstance(connection, dict) and api_client is not None:
            from ibm_watsonx_ai.helpers.connections import DataConnection
        ...
        if hasattr(data_connection, "location"):
            try:
                return (
                    data_connection.location._get_file_extension()
                    in expected_extensions
                )
            except NotImplementedError:
                logger.debug(
                    f"TabularIterableDataset: Catched NotImplementedError while checking file extension of DataConnection object: {data_connection}"
                )
                return False
        return False
    ...
    def write(
        ...
        :param data: structured data to be saved in data source connection. (Either 'data' or 'file_path' need to be provided)
        :type data: DataFrame, optional

        :param file_path: path to the local file to be saved in source data connection (binary transfer).
    ...
    def __iter__(self) -> Iterator:
        ...
        if self.authorized:
            if self.read_as_benchmark_data:
                return self.connection.read_benchmarking_data()
